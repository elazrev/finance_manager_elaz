...
export default function QuotePrintPage() {
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center p-8" dir="rtl">
        <p className="text-destructive">הצעה לא נמצאה</p>
      </div>
    );
  }
  ...
  const wasEdited = quote.updated_at && quote.created_at && new Date(quote.updated_at).getTime() - new Date(quote.created_at).getTime() > 2000;
  ...
  return (
    <div className="print:min-h-0" dir="rtl">
      <div className="fixed top-4 left-4 z-50 print:hidden flex items-center gap-2 sm:gap-4 bg-white/90 dark:bg-black/90 p-3 rounded-lg shadow-lg">
        ...
        <a href={`/quotes/${id}`} className="text-sm text-muted-foreground hover:underline py-2 min-h-[44px] flex items-center touch-manipulation">
          ← חזור
        </a>
      </div>
        ...
        <div className="bg-white text-black text-[13px] sm:text-base print:text-sm">
          ...
          <div className="flex justify-between items-start gap-4 sm:gap-8 mb-4 sm:mb-8 pb-3 sm:pb-4 border-b-2 border-black">
            <div className="text-right min-w-0">
              <h1 className="text-base sm:text-2xl font-bold leading-tight mb-1">הצעת מחיר</h1>
              <p className="text-sm sm:text-xl font-semibold mb-1 sm:mb-2">{quote.quote_number}</p>
              <p className="text-xs sm:text-sm text-gray-700">{formatDate(quote.created_at)}</p>
              {quote.valid_until && (
                <p className="text-xs sm:text-sm text-amber-700 mt-1">תוקף עד: {formatDate(quote.valid_until)}</p>
              )}
            </div>
          ...
          <div className="flex justify-end mt-4 sm:mt-6">
            <div className="text-right border-t-2 border-black pt-2 sm:pt-3 min-w-[140px] sm:min-w-[200px] space-y-1 text-xs sm:text-sm">
              <div className="flex justify-between gap-4 sm:gap-6">
                ...
                <span className="tabular-nums font-medium">{formatCurrency(quote.total ?? 0)}</span>
              </div>
              <div className="flex justify-between gap-4 sm:gap-6">
              ...
              <div className="flex justify-between gap-4 sm:gap-6 pt-2 border-t border-gray-300">
                ...
                <span className="text-base sm:text-xl font-bold tabular-nums">{formatCurrency(quote.total ?? 0)}</span>
              </div>
            ...
            <div className="mt-4 sm:mt-8 pt-3 sm:pt-4 border-t border-gray-300">
              {quote.notes && (
                <>
                  ...
                  <p className="text-xs sm:text-sm leading-relaxed whitespace-pre-wrap">{quote.notes}</p>
                </>
              )}
              {showEditDate && wasEdited && (
                <p className="text-[10px] sm:text-xs text-gray-500 mt-2">נערך לאחרונה: {formatDate(quote.updated_at)}</p>
              )}
            </div>
          )}

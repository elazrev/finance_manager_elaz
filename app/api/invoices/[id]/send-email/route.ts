...
export async function POST(
  ...
  try {
    ...
    const items = (invoice.items || []) as Array<{ description?: string; quantity?: number; unit_price?: number; discount?: number; total?: number }>;
...
<body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px;direction:rtl">
  ...
  <h2 style="margin-bottom:8px">${invoiceType} ${invoice.invoice_number}</h2>
  <p style="color:#666;margin-bottom:24px">מ${businessName} • תאריך: ${formatDate(invoice.created_at)}</p>
  ...
  <p style="margin-top:16px;color:#666;font-size:14px">בברכה,<br>${businessName}</p>
    ...
    const { error: sendError } = await resend.emails.send({
      ...
      to: clientEmail,
      subject: `${invoiceType} ${invoice.invoice_number} - ${formatCurrency(total)}`,
      html,
    });
